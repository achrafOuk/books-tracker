...
@foreach($comments as $comment)
    <article class="my-6 rounded-lg bg-white text-base" x-data="{ edit:false}">
      <div x-show="!edit">
        ...
        <div class="flex items-center mt-2 space-x-4">
            @if( Auth::user()->id ==   $comment->user->id  )
              <button class="text-sm text-blue-500 hover:underline" x-on:click="edit=true">Edit</button>
      ...
      <div x-show="edit">
        <x-comment-edit-form :rating="$comment->rating" :comment="$comment->comment" :book_slug="$book_slug" />
      </div>
