@props(['book_slug','rating','comment'])
<form method="POST" action="{{ route('update-book-comment',['slug'=>$book_slug]) }}">
    @csrf
    <div class="mb-4 flex">
        <textarea name="comment" class="w-full  rounded-lg border border-gray-300 px-4 py-2 focus:outline-none" placeholder="Enter your comment">{{$comment}}</textarea>
    </div>

    <div class="mb-4 flex items-center">
        <span class="mr-2">Rating:</span>
        <div class="rating flex items-center" x-data="{ rating: {{$rating}} }">
